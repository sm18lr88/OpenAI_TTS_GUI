...
def _read_src(*parts: str) -> str:
    return (
        Path(__file__).resolve().parents[1] / "src" / "openai_tts_gui" / Path(*parts)
    ).read_text(encoding="utf-8")


def test_keyring_service_name():
    source = _read_src("keystore", "_storage.py")
...
def test_env_var_names():
    keystore_source = _read_src("keystore", "_storage.py")
    config_source = _read_src("config", "settings.py")

    assert "OPENAI_API_KEY" in keystore_source
